...
describe('Pawn', () => {
    ...
    describe('black pawn', () => {
        ...
        describe('when there is no piece in front', () => {
            it('when pawn has not been moved', () => {
                expect(pawn._movesWithoutCapture()).toEqual([point('a6'), point('a5')])
                expect(pawn._movesWithCapture()).toEqual([])
            })

            it('when pawn has been moved', () => {
                pawn.moved = true

                expect(pawn._movesWithoutCapture()).toEqual([point('a6')])
                expect(pawn._movesWithCapture()).toEqual([])
            })
        })
        ...
        it('when there is a friendly piece in front', () => {
            ...
            expect(pawn._movesWithoutCapture()).toEqual([])
            expect(pawn._movesWithCapture()).toEqual([])
        })
        ...
        it('when there is an enemy piece diagonally 1 tile away from and in front of the pawn', () => {
            ...
            expect(pawn._movesWithoutCapture()).toEqual([point('a6'), point('a5')])
            expect(pawn._movesWithCapture()).toEqual([point('b6')])
        })
    ...
    describe('white pawn', () => {
        ...
        it('when pawn has not been moved', () => {
            expect(pawn._movesWithoutCapture()).toEqual([point('a3'), point('a4')])
            expect(pawn._movesWithCapture()).toEqual([])
        })
        ...
        it('when pawn is on a3', () => {
            ...
            pawn.moved = true
            expect(pawn._movesWithoutCapture()).toEqual([point('a4')])
            expect(pawn._movesWithCapture()).toEqual([])
        })
